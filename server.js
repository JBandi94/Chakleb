...
// Proxy TikTok avatar images through the server. TikTok CDN URLs can be
// inaccessible/expired from the browser directly, so the game loads them
// from the same Render origin.
...
function extractProfilePictureUrl(data) {
    ...
    const candidates = [
        ...
        data?.avatarLarger, data?.avatarMedium, data?.avatarThumb,
        user?.avatar_url, data?.avatar_url
    ];
    const value = candidates.find(v => typeof v === 'string' && /^https?:\/\//i.test(v.trim()));
